...
module Hadoop.Protos.DataTransferProtos.ChecksumProto (ChecksumProto(..)) where
import Prelude ((+), (/))
...
data ChecksumProto = ChecksumProto{type' :: !(HdfsProtos.ChecksumTypeProto), bytesPerChecksum :: !(P'.Word32)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable ChecksumProto where
  mergeAppend (ChecksumProto x'1 x'2) (ChecksumProto y'1 y'2) = ChecksumProto (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default ChecksumProto where
  defaultValue = ChecksumProto P'.defaultValue P'.defaultValue
 
instance P'.Wire ChecksumProto where
  wireSize ft' self'@(ChecksumProto x'1 x'2)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeReq 1 14 x'1 + P'.wireSizeReq 1 13 x'2)
  wirePut ft' self'@(ChecksumProto x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
...
instance P'.TextMsg ChecksumProto where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'type', parse'bytesPerChecksum]) P'.spaces
