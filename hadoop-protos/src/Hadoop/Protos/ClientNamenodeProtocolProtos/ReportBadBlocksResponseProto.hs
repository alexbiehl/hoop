...
module Hadoop.Protos.ClientNamenodeProtocolProtos.ReportBadBlocksResponseProto (ReportBadBlocksResponseProto(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor ReportBadBlocksResponseProto where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".hadoop.hdfs.ReportBadBlocksResponseProto\", haskellPrefix = [MName \"Hadoop\",MName \"Protos\"], parentModule = [MName \"ClientNamenodeProtocolProtos\"], baseName = MName \"ReportBadBlocksResponseProto\"}, descFilePath = [\"Hadoop\",\"Protos\",\"ClientNamenodeProtocolProtos\",\"ReportBadBlocksResponseProto.hs\"], isGroup = False, fields = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"
